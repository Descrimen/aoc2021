...
full_ranges_s = [list(zip(range(x[0],x[2]+1),[x[1]]*(x[2]-x[0]+1))) if (x[1]==x[3]) else list(zip([x[0]]*(x[3]-x[1]+1),range(x[1],x[3]+1))) for x in ordered_coords]

diagonal_lines = [c for c in coords if (c[0]!=c[2] and c[1]!=c[3])]
full_ranges_d = [list(zip(range(x[0],x[2]+1),range(x[1],x[3]+1))) if (x[2]>x[0] and x[3]>x[1]) else list(zip(range(x[0],x[2]-1,-1),range(x[1],x[3]+1))) if (x[2]<x[0] and x[3]>x[1]) else list(zip(range(x[0],x[2]-1,-1),range(x[1],x[3]-1,-1))) if (x[2]<x[0] and x[3]<x[1]) else list(zip(range(x[0],x[2]+1),range(x[1],x[3]-1,-1))) for x in diagonal_lines]

full_ranges = full_ranges_s + full_ranges_d
